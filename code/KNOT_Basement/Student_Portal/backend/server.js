...
app.post('/api/bookings', async (req, res) => {
  const { title, time_display, user_id, icon, status, end_time, assigned_lecturer, purpose, booking_type } = req.body;
  const connection = await pool.getConnection();
  try {
    await connection.beginTransaction();

    // Determine booking request type: 'AR Office' vs 'Lecture'
    let requestType = booking_type;
    if (!requestType) {
      if (assigned_lecturer) {
        requestType = 'Lecture';
      } else if (user_id) {
        const [uRows] = await connection.query('SELECT role, department FROM users WHERE id = ?', [user_id]);
        if (uRows.length > 0) {
          const u = uRows[0];
          if (u.role === 'Lecturer' || (u.department && u.department !== 'AR Office' && u.department !== 'Facilities Management')) {
            requestType = 'Lecture';
          } else {
    ...
    // Step 1 (Mandatory Manual): All new requests (status === 'Pending') MUST go to Lecture approval stage without automated intervention.
    // Only if Step 1 is ALREADY complete (e.g. status === 'Pending AR', such as direct bookings by a Lecturer) do we evaluate Step 2 conditional AR approval.
    ...
    // Manual Flow / Step 1: Insert into database waiting for manual Lecture-level approval (status = 'Pending') or AR review (status = 'Pending AR')
    const [result] = await connection.query(
